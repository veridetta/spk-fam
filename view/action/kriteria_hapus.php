...
if(isset($_SESSION['user_id'])) {
    ...
    if($role == 'admin') {
        $id = $_GET['id'];
        // Query SQL untuk hapus data kriteria
        $sql = "DELETE FROM kriteria WHERE id = '$id'";
        if ($koneksi->query($sql) === TRUE) {
            $_SESSION['pesan'] = array('jenis' => 'success', 'pesan' => 'Data kriteria berhasil dihapus.');
            header("Location: ../index.php?page=admin_kriteria");
            exit();
        } else {
            $_SESSION['pesan'] = array('jenis' => 'danger', 'pesan' => 'Data kriteria gagal dihapus.');
            header("Location: ../index.php?page=admin_kriteria");
            exit();
        }
